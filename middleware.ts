...
export function paymentMiddleware(
    ...
    const { settle, supported } = useFacilitator(facilitator);
    const x402Version = 1;
        ...
        try {
            ...
            if (!settlement.success) {
                return new NextResponse(
                    JSON.stringify({
                        x402Version,
                        error: errorMessages?.settlementFailed || "Payment settlement failed",
                        accepts: paymentRequirements,
                    }),
                    { status: 402, headers: { "Content-Type": "application/json" } },
                );
...
export const middleware = paymentMiddleware(
    ...
    {
        url: (process.env.FACILITATOR_URL || 'https://facilitator.payai.network') as `${string}://${string}`,
    }
);
